...
export function selectEvidence(documents, focus = '') {
  ...
  // Reserve room for recent owner-saved inspiration without displacing category coverage.
  for (const doc of ranked.filter(d => d.title.startsWith('Saved reference:')).slice(0, 3)) {
    if (!chosen.some(c => c.id === doc.id)) chosen.push(doc);
  }
...
General trend provider snapshots are secondary evidence. Read request settings, fetched_at, as_of, publication dates and growth baseline/recent dates separately. Provider-cached data may predate retrieval. Broad popular feeds are not fashion/beauty-specific; select only relevant topics and explain audience fit. SocialCrawl search is not proof of trend growth. TrendsAPI.ai normalized demand and reported growth are not sales or conversion rates. Cite outside evidence if used and support its fit with the owner's evidence. If source dates are missing, old or inconsistent, state that and do not claim a current verified trend.
Outside creator/hashtag examples and owner-saved references are secondary inspiration; your own results, assets, preferences and commitments remain primary. Explicitly connect any outside idea to first-party evidence and explain a distinct adaptation, not copying. A hashtag top-media result is not community-wide momentum. A single snapshot cannot prove acceleration or outperformance relative to a creator baseline. Repeated captions across posts do not prove repeated independent creators; source overlap is deduplicated. Do not infer competitor sales, conversions, private saves/watch time, rights or audio licenses. Use 'recent observed example' or 'hypothesis to test' unless dated repeated observations actually establish the claimed trend. Saved references may be stale and are not independently fetched. Never invent trend evidence when sources are absent. If outside examples influence your recommendation, cite their exact source excerpts alongside relevant first-party evidence.
